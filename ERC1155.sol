...
contract ERC1155 is IERC1155, IERC1155MetadataURI {
    ...
    function balanceOfBatch(address[] calldata accounts, uint[] calldata ids) public view returns(uint[] memory batchBalances) {
    ...
    function _doSafeTransferAcceptanceCheck(
        ...
        if(to.code.length > 0) {
            try IERC1155Receiver(to).onRERC1155Received(operator, from, id, amount, data) returns(bytes4 resp) {
                if(resp != IERC1155Receiver.onRERC1155Received.selector) {
                    revert("rejected!");
                }
    ...
    function _doSafeBatchTransferAcceptanceCheck(
        ...
        if(to.code.length > 0) {
            try IERC1155Receiver(to).onRERC1155BatchReceived(operator, from, ids, amounts, data) returns(bytes4 resp) {
                if(resp != IERC1155Receiver.onRERC1155BatchReceived.selector) {
                    revert("rejected!");
                }
            } catch Error(string memory reason) {
                revert(reason);
